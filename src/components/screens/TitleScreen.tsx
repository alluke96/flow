...
export function TitleScreen({ id }: { id: string }) {
  return (
    <RequireProfile>
      <TitleDetailInner id={id} />
    </RequireProfile>
  );
}

function TitleDetailInner({ id }: { id: string }) {
  const { ir } = useNav();
  ...
  if (notFound || !title) {
    return (
      <div style={{ padding: 60, textAlign: "center" }}>
        ...
        <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => ir({ nome: "browse" })}>
          Voltar
        </button>
      </div>
    );
  ...
  return (
    <div>
      <div className="detail-banner">
        <button className="back-btn" onClick={() => ir({ nome: "browse" })} aria-label="Voltar">
          <span>←</span>
        </button>
              ...
              <div key={seasonIdx} className="episode-list">
                {season?.episodios.map((ep, i) => (
                  <button
                    key={ep.id}
                    className="episode"
                    onClick={() => ir({ nome: "player", id: title.id, ep: ep.id })}
                  >
