...
export function useFullscreen(
  ...
  // Opcional: quando passado, é usado no lugar de v.closest(".player-shell")
  // pra achar o container. Precisa disto pro modo player nativo AVPlay (ver
  // VideoPlayer.tsx), que não tem <video> nenhum pra buscar o container a
  // partir dele.
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    //
    // containerRef, quando dado, prevalece sobre v.closest(...) — é o caso
    // do modo player nativo AVPlay, sem <video> nenhum pra buscar a partir
    // dele (ver VideoPlayer.tsx).
    const v = videoRef.current as WebkitVideoElement | null;
    const container = containerRef?.current ?? v?.closest(".player-shell");
    ...
    // Só chega aqui em navegador sem Fullscreen API padrão no container
    // (ex: iOS Safari) — o modo nativo nunca ativa fora da casca Tizen, que
    // tem a API padrão, então sempre resolve no ramo acima.
    if (!v) return;
