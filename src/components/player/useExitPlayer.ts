...
export function useExitPlayer({
  ...
  onExit: () => void;
  syncProfiles: () => void;
  // Opcional: quando dado, é chamado no lugar da liberação padrão do
  // <video> (pause/removeAttribute/load) — é o caso do modo player nativo
  // AVPlay (ver VideoPlayer.tsx), que não tem <video> nenhum pra soltar;
  // ali a mídia de verdade é fechada na casca Tizen (tz.api.close()).
  releaseMedia?: () => void;
}) {
  ...
  }, [videoRef, doSaveProgress, onExit, syncProfiles, releaseMedia]);
